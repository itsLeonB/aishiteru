...
func newOpenRouterService(apiKey, model string) LLMService {
	if apiKey == "" {
		logging.Logger.Fatalf("missing OpenRouter API Key")
	}
	if model == "" {
		logging.Logger.Fatalf("OpenRouter model is not specified")
	}

	client, err := openroutergo.NewClient().
		WithAPIKey(apiKey).
		WithRefererTitle("Aishiteru").
		Create()

	if err != nil {
		logging.Logger.Fatalf("error creating open router client: %v", err)
	}
